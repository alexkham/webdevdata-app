// Emulator for Python bytearray(source, encoding) from a string source.
...
export default function pyBytearray(source, encoding = null) {
  if (typeof source !== 'string') throw new TypeError('bytearray() demo source must be str');
  if (encoding === null || encoding === undefined) {
    throw new TypeError('string argument without an encoding');
  }
  return { __pyRaw: 'bytearray(' + bytesRepr(encodeBytes(source, encoding)) + ')' };
}
