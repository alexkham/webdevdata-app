// pages/reference/python/operators/[name].jsx
//
// Dynamic leaf for /reference/python/operators/<slug>.
// Same shape as the functions leaf — different content folder, operator
// category labels, and the operators emulator namespace.
...
export async function getStaticPaths() {
  ...
  const dir = path.join(process.cwd(), 'content/reference/python/operators');
...
export async function getStaticProps({ params }) {
  const { meta, method } = require('@/content/reference/python/operators/' + params.name);
  const { pythonOperatorsCatalog } = require('@/data/generated/python-operators-catalog');

  const siblings = pythonOperatorsCatalog.items
  ...
  const seoData = {
    title:       `Python ${meta.name} Operator — Reference | ${SITE_NAME}`,
    description: `${meta.blurb} Syntax, examples, pitfalls${meta.hasLiveDemo ? ' and a live in-browser demo' : ''} of the Python ${meta.name} operator.`,
    name:        meta.name,
  ...
  const schemas = {
    article: {
      ...
      '@type':       'TechArticle',
      headline:      `Python ${meta.name} operator`,
...
export default function OperatorPage({ seoData, meta, method, siblings, siblingsTitle, schemas, frameOptions }) {
  ...
  if (method.parameters && method.parameters.length > 0) {
    sections.push({ id: 'params', label: 'Operands', count: null, content: <Parameters parameters={method.parameters} returns={method.returns} /> });
  }
