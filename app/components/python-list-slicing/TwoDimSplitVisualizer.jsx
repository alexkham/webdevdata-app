...
function TwoDimListSplitVisualizer() {
    ...
    useEffect(() => {
        setOrderHorizontal(stepCol < 0 ? 'reverse' : 'original');
    }, [stepCol]);

    useEffect(() => {
        setOrderVertical(stepRow < 0 ? 'reverse' : 'original');
    }, [stepRow]);

    useEffect(() => {
        calculateHighlightedIndices();
        const slicedData = getSlicedData();
        setResult(slicedData);
    }, [startRow, stopRow, stepRow, startCol, stopCol, stepCol, data]);
    ...
    function getSlicedData() {
        ...
        const slicedData = rowIndices.map(r => 
            colIndices.map(c => data[r][c])
        );
    ...
    return (
        <div className='outer-container-two-dim'>
            <div className='array-visualization-container'>
                <div className='row-block'>
                    {orderVertical && (
                        <div className='order-vertical' style={{fontSize:'300'}}>
                            {orderVertical === 'original' ? (
                                <>
                                    <p>Original Order</p>
                                    <p className='arrow'>↓</p>
                                </>
                ...
                {orderHorizontal && (
                    <div className='order' style={{fontSize:'20'}}>
                        {orderHorizontal === 'original' ? `         Original Order →` : '← Reverse Order'}
                    </div>
                )}
            </div>
