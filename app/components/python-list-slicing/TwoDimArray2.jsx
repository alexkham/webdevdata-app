...
// function TwoDimArray2({ data: externalData, highlightedIndices, shouldHighlight }) {
...
//   // Generate default data if external data is not provided
//   useEffect(() => {
//     if (!externalData || externalData.length === 0) {
//       const defaultData = Array.from({ length: 10 }, (_, i) =>
//         Array.from({ length: 10 }, (_, j) => 10 * i  + j )
//       );
//       setData(defaultData);
//     } else {
...
//   // Calculate the total height of the table based on data
//   useEffect(() => {
//     const rowHeight = 25; // Base height per row
//     const spacing = 5;  // Additional spacing for between rows
//     const totalHeight = data.length * rowHeight + (data.length - 1) * spacing; // Calculate total height including inter-row space
...
//                 <div className="indices">
//                   <div style={{ fontSize: '12px', marginLeft: '0px' }}>{colIndex}</div>
//                   <div style={{ fontSize: '12px', marginRight: '5px' }}>{-(data[data.length - 1].length - colIndex )}</div>
//                 </div>
//               </td>
//             ))} */}
...
function TwoDimArray2({ data: externalData, highlightedIndices, shouldHighlight }) {
  ...
  useEffect(() => {
    const rowHeight = 25; // Base height per row
    const spacing = 5;  // Additional spacing for between rows
    const totalHeight = externalData.length * rowHeight + (externalData.length - 1) * spacing; // Calculate total height including inter-row space
    setTableHeight(totalHeight);
  }, [externalData]);

  return (
    <div className='outer-frame' style={{ display: 'flex', alignItems: 'center' }}>
      <div className='main-bracket' style={{ fontSize: `${tableHeight}px`, height: `${tableHeight}px`, marginBottom: `${externalData.length * 10}px` }}>[</div>
      <table className="python-list" cellSpacing="0" cellPadding="0" style={{ borderCollapse: 'collapse', tableLayout: 'fixed' }}>
        <tbody>
          {externalData.map((row, rowIndex) => (
            <tr key={rowIndex}>
              <td style={{ padding: 0 }}>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginRight: '5px' }}>
                  <div style={{ display: 'flex', flexDirection: 'row' }}>
                    <div style={{ fontSize: '12px', marginRight: '25px' }}>{-externalData.length + rowIndex}</div>
                    <div style={{ fontSize: '12px' }}>{rowIndex}</div>
                  </div>
                </div>
              </td>
              ...
              {row.map((item, colIndex) => (
                <td key={colIndex} style={{ padding: 0 }}>
                  <div style={shouldHighlight && highlightedIndices?.some(([r, c]) => r === rowIndex && c === colIndex) ? highlightedCellStyle : cellStyle}>
                    {item}
                  </div>
                </td>
              ))}
          ...
          {externalData.length > 0 && (
            <tr>
              <td></td>
              <td></td>
              {externalData[externalData.length - 1].map((_, colIndex) => (
                <td key={colIndex} style={{ textAlign: 'center', padding: '0px' }}>
                  <div className="indices">
                    <div style={{ fontSize: '12px', marginLeft: '0px' }}>{colIndex}</div>
                    <div style={{ fontSize: '12px', marginRight: '5px' }}>{-(externalData[externalData.length - 1].length - colIndex)}</div>
                  </div>
                </td>
              ))}
              <td></td>
