// JsBeautifierTool.jsx — v1
// JavaScript beautifier / unminifier using js-beautify (dynamic-imported on first use).
// Single-mode tool per WebDevData tool-component-guide-v2.
//
// Requires: `npm i js-beautify`
...
const DEFAULT_EXPLANATIONS = {
  beautify: (
    <>
      <p><b>Beautify</b> reformats compact or minified JavaScript into a readable form — line breaks, consistent indentation, spacing around operators.</p>
      <p>Uses <code>js-beautify</code>, the same library behind VS Code&apos;s HTML/CSS/JS formatter fallback and countless online tools.</p>
    </>
  ),
};
...
const SAMPLES = [
  {
    label: 'Simple minified',
    tt: 'A short minified function.',
    text: `function add(a,b){return a+b}const result=add(2,3);console.log(result);`,
  },
  {
    label: 'One-liner',
    tt: 'Everything on a single line — common in embed snippets.',
    text: `(function(){var a=[1,2,3];var b=a.map(function(x){return x*2});console.log(b.reduce(function(s,x){return s+x},0))})();`,
  },
  {
    label: 'Minified class',
    tt: 'Minified ES6 class.',
    text: `class Counter{constructor(n=0){this.count=n}increment(){return++this.count}reset(){this.count=0}}const c=new Counter(10);c.increment();console.log(c.count);`,
  },
  {
    label: 'Real-world blob',
    tt: 'The kind of thing you paste from a script tag.',
    text: `!function(e,t){"object"==typeof exports&&"undefined"!=typeof module?module.exports=t():"function"==typeof define&&define.amd?define(t):(e="undefined"!=typeof globalThis?globalThis:e||self).lib=t()}(this,(function(){"use strict";return{sum:function(e){return e.reduce((function(e,t){return e+t}),0)},avg:function(e){return e.length?this.sum(e)/e.length:0}}}));`,
  },
];
...
export default function JsBeautifierTool({
  ...
  const [maxPreserveNewlines, setMaxPreserveNewlines] = useState(2);
  const [spaceInParen, setSpaceInParen] = useState(false);
  const [endWithNewline, setEndWithNewline] = useState(true);
  const [breakChainedMethods, setBreakChainedMethods] = useState(false);
  ...
  const ensureBeautify = async () => {
    if (!beautifyRef.current) {
      const mod = await import('js-beautify');
      // js-beautify's default export is the JS beautifier fn; also exposes .js
      beautifyRef.current = mod.js || mod.default || mod;
    }
    return beautifyRef.current;
  };

  // Live beautify (debounced 300ms)
  useEffect(() => {
    if (!input.trim()) {
      setOutput('');
      setError('');
    ...
    const timer = setTimeout(async () => {
      try {
        setBusy(true);
        const beautify = await ensureBeautify();

        const opts = {
          indent_size: indentSize === 'tab' ? 1 : indentSize,
  ...
  }, [input, indentSize, preserveNewlines, maxPreserveNewlines, spaceInParen, endWithNewline, breakChainedMethods]);
  ...
  const handleReset = useCallback(() => {
    ...
    setMaxPreserveNewlines(2);
    setSpaceInParen(false);
    setEndWithNewline(true);
    setBreakChainedMethods(false);
  }, []);
  ...
  const linesIn = input ? input.split('\n').length : 0;
  const linesOut = output ? output.split('\n').length : 0;
  ...
  return (
    <div className="tool" data-theme={currentTheme}>
      ...
      {/* options row */}
      <div className="opts">
        <span className="opt-select" data-tt="Spaces or tabs used to indent output">
          <span className="opt-select-lbl">Indent:</span>
          <select
            value={indentSize}
            onChange={(e) => {
              const v = e.target.value;
              setIndentSize(v === 'tab' ? 'tab' : parseInt(v, 10));
            }}
          >
        ...
        <label className="opt" data-tt="Keep blank lines from the input in the output">
          <input
            type="checkbox"
        ...
        <span className="opt-select" data-tt="Maximum consecutive blank lines to keep">
          <span className="opt-select-lbl">Max blank:</span>
          <select
            value={maxPreserveNewlines}
            onChange={(e) => setMaxPreserveNewlines(parseInt(e.target.value, 10))}
        ...
        <label className="opt" data-tt="Space between parentheses and their contents: ( x ) instead of (x)">
          <input
            type="checkbox"
            checked={spaceInParen}
            onChange={(e) => setSpaceInParen(e.target.checked)}
          />
          Space in parens
        </label>

        <label className="opt" data-tt="Break each chained .method() onto its own line">
          <input
            type="checkbox"
            checked={breakChainedMethods}
            onChange={(e) => setBreakChainedMethods(e.target.checked)}
          />
          Break chained methods
        </label>

        <label className="opt" data-tt="End the file with a trailing newline (POSIX-friendly)">
          <input
            type="checkbox"
            checked={endWithNewline}
            onChange={(e) => setEndWithNewline(e.target.checked)}
          />
          Trailing newline
        </label>
      </div>
      ...
      <div className={'body' + (showExplanations ? ' with-exp' : '')}>
        <div className={'panes ' + orientation}>
          ...
          <div className="pane">
            <div className="lbl">
              <span>Beautified Output {busy && <em className="busy">formatting…</em>}</span>
              <div className="lbl-actions">
                ...
                <button
                  ...
                  disabled={!output || !!error}
                  data-tt="Save output as beautified.js"
                  data-tt-pos="above"
                >
                  Download
                </button>
            ...
            <div className={'out' + (error ? ' err' : '')}>
              {error || output || <span style={{ color: 'var(--text-input-placeholder)' }}>Beautified output will appear here.</span>}
            </div>
          </div>
        </div>
      ...
      {/* stats */}
      <div className="stats">
        <span>Input: <b>{inBytes}</b> bytes &middot; <b>{linesIn}</b> lines</span>
        <span>Output: <b>{outBytes}</b> bytes &middot; <b>{linesOut}</b> lines</span>
